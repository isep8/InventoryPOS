...
try
    ...
    begin
        ...
        //validate
        if trim(cboDestinationName.Text)='' then
        begin
            messageDlg('Please select destination name.',mtInformation,[mbOk],0);
            cboDestinationName.Text;
            cboDestinationName.SetFocus;
            Exit;
        end;
        ...
        if cboDestinationName.Text='ALL' then
        begin
        ...
        begin
            ...
            begin
                ...
                SQL.Clear;
                SQL.Add('Select * From StockOutDetail' +  strStockOutTBL );
                SQL.Add('Where (EnteredDate>=:asEnteredDate1 and EnteredDate<=:asEnteredDate2) and Upper(DestinationName)=:asDestinationName');
                SQL.Add('Order by TransType, EnteredDate, EnteredTime, Description');
                ParamByName('asEnteredDate1').Value:= StrToDate(FormatDateTime('mm/dd/yyyy', dtFrom.Date));
                ParamByName('asEnteredDate2').Value:= StrToDate(FormatDateTime('mm/dd/yyyy', dtTo.Date));
                ParamByName('asDestinationName').Value:= trim(cboDestinationName.Text);
                open;
            end;
            rpt1.SetParam('Header1', frmMain.edtHeader1.Text);
            rpt1.SetParam('Header2', frmMain.edtHeader2.Text);
...
begin
    ...
    begin
        strStockOutTBL:= Format('%.*d',[2, yearof(dtFrom.Date)]) + Format('%.*d',[2, monthof(dtFrom.Date)]);
        strStockOutTBL:= 'StockOutDetail' + strStockOutTBL;
        strStockCode:= LeftStr(cboDestinationName.Text,10);
        ...
        begin
            Close;
            SQL.clear;
            SQL.Add('Select DestinationName From ' + strStockOutTBL );
            SQL.Add('Group by DestinationName');
            SQL.Add('Order by DestinationName');
            Open;
        end;

        If (not qry2Chk.Active) or (qry2Chk.RecordCount = 0) then
        begin
            //message
            cboDestinationName.Text:= 'NO RECORD';
            exit;
        end;

        cboDestinationName.Items.Clear;
        cboDestinationName.Items.Add('ALL');
        qry2Chk.First;
        while not qry2Chk.Eof do
        begin
            cboDestinationName.Items.Add(qry2Chk.fieldByName('DestinationName').Value);
        qry2Chk.Next;
        end;
        qry2Chk.Close;
